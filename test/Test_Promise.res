...
describe("Promise", () => {
  let empty = () => Promise.make((resolve, _) => resolve(true))

  /* Calls given function after a small delay */
  let delay = fn =>
    Promise.make((resolve, _) => setTimeout(() => {
          fn()
          resolve(true)
        }, 300)->ignore)
  ...
  describe("Timeout", () => {
    it_skip("should time out", ~timeout=50, () =>
      Promise.make((resolve, _) =>
        setTimeout(() => resolve(true), 51)->ignore
      )
    )

    it("should not time out", ~timeout=50, () =>
      Promise.make((resolve, _) =>
        setTimeout(() => resolve(true), 40)->ignore)
    )
  })
  ...
  describe("Slow", () =>
    it("should be considered slow", ~slow=50, () =>
      Promise.make((resolve, _) =>
        setTimeout(() => resolve(true), 60)->ignore
      )
    )
  )
})
