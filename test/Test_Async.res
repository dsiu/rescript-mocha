...
describe("Async", () => {
  ...
  let delay = fn => setTimeout(() => fn(), 300)->ignore

  describe("Success", () =>
    it("should be successful", done_ => setTimeout(() => {
            Assert.equal(3, 3)
            done_()
          }, 500)->ignore)
  )
  ...
  describe("Timeout", () => {
    it_skip("should time out", ~timeout=50, done_ => setTimeout(() => done_(), 51)->ignore)

    it("should not time out", ~timeout=50, done_ => setTimeout(() => done_(), 40)->ignore)
  })
  ...
  describe("Slow", () =>
    it("should be considered slow", ~slow=50, done_ => setTimeout(() => done_(), 40)->ignore)
  )
})
